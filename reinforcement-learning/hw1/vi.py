...
def value_iteration(env, gamma, max_iteration, tol):
    ...
    for n in range(max_iteration):
        delta = 0
        temp = V[:]
        # adapt utilities based on neighbors
        for i in range (env.nS):
            t = V[i]
            V[i] = max([env.P[i][j][0][2] + gamma * temp[env.P[i][j][0][1]] for j in range (6)])
            if delta < np.abs(t - V[i]):
                delta = np.abs(t - V[i])
        
        # break when converge
        if delta < tol:
            print("[INFO] Value interation converges at {} iteration.".format(n))
            break
