...
class PlanningGraph:
            ...
            for literal in self.literal_layers[layer]:
                ...
                for goal in goal_list:
                    ...
                    if goal.__eq__(literal):
                        ...
                        level_sum = level_sum + layer
                        # remove goal from goalList so it is not checked twice.
                        goal_list = [x for x in goal_list if x is not goal]
            layer = layer + 1
            self._extend()
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            if set(goal_list).issubset(self.literal_layers[layer]):
                is_goal_mutex = False
                for goal in goal_list:
                    for otherGoal in goal_list:
                        if self.literal_layers[layer].is_mutex(goal, otherGoal):
                            is_goal_mutex = True
                if not is_goal_mutex:
                    set_level = layer
                    break
            layer = layer + 1
            self._extend()
